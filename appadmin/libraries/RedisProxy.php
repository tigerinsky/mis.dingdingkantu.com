...
class RedisProxy {
    ...
    public function delete($key) {
        try {
            $ret = $this->_redis->delete($key);
            $err = $this->_redis->getLastError();
            if (false === $err) {
                log_message('error', 'redis err delete.');
                return false;
            }
            return $ret;
        } catch (Exception $e) {
    ...
    public function exists($key) {
        try {
            ...
            $err = $this->_redis->getLastError();
            if (false === $err) {
                log_message('error', 'redis error exists.');
                return false;
            }
            return $ret;
        } catch (Exception $e) {
            log_messge('error', 'redis exception exist: ['.$e->getMessage().']');
            return false;
        }
        return false;
    }
    ...
    public function sadd($key, $member) {
        try {
            $ret = $this->_redis->sAdd($key, $member);
            $err = $this->_redis->getLastError();
            if (false === $err) {
                log_message('error', 'redis error sadd['.$err.']');
            }
            return $ret;
        } catch (Exception $e) {
            log_message('error', 'redis exception sadd ['.$e->getMessage().']');
            return false;
        }
        return false;
    }
    ...
    public function sismember($key, $member) {
        try {
            $ret = $this->_redis->sismember($key, $member);
            $err = $this->_redis->getLastError();
            if (false === $err) {
                log_message('error', 'redis error sismember.');
                return false;
            }
            return $ret;
        } catch (Exception $e) {
            log_message('error', 'redis error sismember: ['.$e->getMessage().']');
            return false;
        }
        return false;
    }
